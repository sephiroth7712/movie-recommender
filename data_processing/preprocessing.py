    ...
    def load_plot_summaries(self) -> pd.DataFrame:
        ...
        with open(self.config.plot_summaries_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                plot_data.append({'wikipedia_movie_id': int(movie_id), 'plot_summary': plot})
        return pd.DataFrame(plot_data)
    
    def load_movie_metadata(self) -> pd.DataFrame:
        """Load and process movie metadata"""
        logger.info("Loading CMU movie metadata...")
        columns = ['wikipedia_movie_id', 'freebase_movie_id', 'movie_name', 
                  'release_date', 'box_office_revenue', 'runtime', 
                  'languages', 'countries', 'genres']
        
        metadata_df = pd.read_csv(self.config.movie_metadata_path, 
                                sep='\t', names=columns)
        
        # Convert wikipedia_movie_id to int
        metadata_df['wikipedia_movie_id'] = metadata_df['wikipedia_movie_id'].astype(int)
        
        # Process JSON-like strings in relevant columns
        metadata_df['genres'] = metadata_df['genres'].apply(self._parse_genres)
        metadata_df['languages'] = metadata_df['languages'].apply(self._parse_freebase_string)
        metadata_df['countries'] = metadata_df['countries'].apply(self._parse_freebase_string)
...
class IMDbLoader:
    ...
    def load_title_basics(self) -> pd.DataFrame:
        ...
        df = pd.read_csv(self.config.title_basics_path, 
    ...
    def load_title_ratings(self) -> pd.DataFrame:
        ...
        df = pd.read_csv(self.config.title_ratings_path, 
                        sep='\t', compression='gzip',
                        na_values='\\N')
...
class DatasetIntegrator:
    ...
    def create_genre_classification_dataset(self) -> pd.DataFrame:
        ...
        try:
            ...
            genre_df = plot_summaries.merge(cmu_metadata, on='wikipedia_movie_id', how='inner')
            
            logger.info(f"Merged dataset shape: {genre_df.shape}")
            
            # Select relevant columns
            genre_df = genre_df[['wikipedia_movie_id', 'movie_name', 
                               'plot_summary', 'genres']]
    ...
    def create_recommendation_dataset(self) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
        ...
        try:
            ...
            movies_enriched = (movies_enriched
                             .merge(imdb_basics[['tconst', 'runtimeMinutes', 'genres']],
                                   left_on='imdbId', right_on='tconst', how='left',
                                   suffixes=('_movielens', '_imdb'))
                             .merge(imdb_ratings[['tconst', 'averageRating', 'numVotes']],
                                   on='tconst', how='left'))
            ...
            logger.info(f"Average ratings per movie: {len(ratings) / len(movies_enriched):.2f}")
            logger.info("\nSample of processed movies:")
            logger.info(movies_enriched[['title', 'year', 'average_rating', 
                                       'number_of_ratings', 'imdb_rating', 
                                       'imdb_votes']].head())
...
def main():
    ...
    try:
        ...
        config = DataPathConfig("./datasets/raw")
        ...
        output_path = Path("./datasets/processed_data")
        output_path.mkdir(exist_ok=True)
        
        # Convert list columns to strings for CSV storage
        movies_enriched['genres'] = movies_enriched['genres'].apply(lambda x: '|'.join(x) if isinstance(x, list) else x)
        movies_enriched['imdb_genres'] = movies_enriched['imdb_genres'].apply(lambda x: '|'.join(x) if isinstance(x, list) else x)
