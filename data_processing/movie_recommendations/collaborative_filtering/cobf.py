...
class CollaborativeRecommender:
    ...
    def fit(self, ratings_df: pd.DataFrame, movies_df: pd.DataFrame) -> None:
        ...
        self.logger.info(
            f"Model fitted with {len(unique_users)} users and {len(unique_movies)} movies"
        )
    ...
    def _get_user_vector(self, user_ratings: Dict[int, float]) -> csr_matrix:
        ...
        return csr_matrix(
            (data, (np.zeros(len(data)), indices)), shape=(1, len(self.movie_id_map))
        )
    ...
    def recommend_movies(
        ...
        try:
            ...
            weighted_ratings = np.average(
                similar_users_matrix.toarray(), axis=0, weights=weights
            )
    ...
    def update_ratings(self, new_ratings: List[Dict[str, any]]) -> bool:
        ...
        try:
            for rating in new_ratings:
                ...
                if (
                    user_idx >= self.user_movie_matrix.shape[0]
                    or movie_idx >= self.user_movie_matrix.shape[1]
                ):
                    new_shape = (
                        max(user_idx + 1, self.user_movie_matrix.shape[0]),
                        max(movie_idx + 1, self.user_movie_matrix.shape[1]),
                    )
                    self.user_movie_matrix = self._resize_sparse_matrix(
                        self.user_movie_matrix, new_shape
                    )
    ...
    def _resize_sparse_matrix(
        self, matrix: csr_matrix, new_shape: Tuple[int, int]
    ) -> csr_matrix:
        """
