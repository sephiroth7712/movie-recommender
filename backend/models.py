from sqlalchemy import Column, Integer, String, Float, Text, ForeignKey, TIMESTAMP, UniqueConstraint
...
class Movie(Base):
    ...
    movie_id = Column(Integer, primary_key=True, index=True, autoincrement=True)  # SERIAL in PostgreSQL
...
class Rating(Base):
    ...
    rating_id = Column(Integer, primary_key=True, autoincrement=True)  # SERIAL in PostgreSQL
    user_id = Column(Integer, ForeignKey("users.id"), nullable=False)  # Foreign key to users(id)
    movie_id = Column(Integer, ForeignKey("movies.movie_id"), nullable=False)  # Foreign key to movies(movie_id)
    rating = Column(Float, nullable=False)  # FLOAT for user ratings
    timestamp = Column(TIMESTAMP, nullable=True)  # TIMESTAMP for when the rating was given
